...
namespace EventStore.Core.Tests.Integration {
	...
	public class when_node_becomes_leader_with_uncommitted_data<TLogFormat, TStreamId> : specification_with_cluster<TLogFormat, TStreamId> {
		...
		public async Task SetUp() {
			...
			for (int i = 0; i < 3; i++) {
				await Helper.EatException(() => _nodes[i].Shutdown(keepDb: false));
			}
		...
		[TestCase(true)]
		[TestCase(false)]
		[Explicit, Category("LongRunning"), Timeout(60000), NonParallelizable]
		public async Task new_events_should_have_correct_event_numbers(bool appendInitialEvent) {
			...
			await Task.WhenAll(_nodes[0].Started, _nodes[1].Started);
